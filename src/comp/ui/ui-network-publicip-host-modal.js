...
@containerless
export class UiNetworkPublicipHostModal {
     ...
     */
    attached() {
        $(this.md).find('table.sortable').tablesort();
        $(this.md).modal({
            closable: false,
            onShow: () => {
    ...
    getHostList() {
        return this.http.fetch(nsApi.url('publicIp.host.listUnbind.get', { pageNo: this.page.currentPage, pageSize: this.page.pageSize, id: this.diskid }))
            .then((resp) => {
                return resp.json();
            }).then((data) => {
